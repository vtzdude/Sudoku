...
def suggest(count,min_moves,difficulty):
    if difficulty=="Easy":
        if(count<=min_moves+5):
            print("You have Mastered this level play Medium level now")
            return "Medium"
        else:
            print("Need more practice,Keep playing")  
            return "Easy"              
    if difficulty == "Medium":
        if(count<=min_moves+5):
            print("You have Mastered this level play Hard level now")
            return "Hard"
        elif(count<=min_moves+15):
            print("Need more practice,Keep playing")
            return "Medium"
        else:
            print("Please play Easy level")
            return "Easy"
    if difficulty == "Hard":
        if(count<=min_moves+5):
            print("You reached god level now")
            return "Hard"
        elif(count<=min_moves+15):
            print("Need more practice,Keep playing")
            return "Hard"
        else:
            print("Please play Medium level")
            return "Medium"
...
def ask_replay(difficulty):
    ch = int(input("Enter 1 to continue\nElse enter 0\n"))
    if ch==1:
        generate(difficulty)
    return    

def Play_Sudoku(Solved_Board,Unsolved_Board,difficulty):
    count=0
    min_moves=count_min_moves(Unsolved_Board)
    while True:    
        ...
        if number_check!=10:
            ...
            if np.array_equal(Solved_Board,Unsolved_Board):
                ...
                print("\nMoves taken : ",count)
                difficulty = suggest(count,min_moves,difficulty)
...
def generate(difficulty):
    ...
    if Solve_Sudoku(board,-1):
        ...
        Play_Sudoku(Solved_Board,Unsolved_Board,difficulty)
    else:
